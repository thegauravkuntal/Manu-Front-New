      ...
      <div className="max-w-7xl mx-auto h-[40px] md:h-[44px] px-4 flex items-center justify-between gap-2 md:gap-6 overflow-hidden">
        ...
        <div className="flex items-center gap-1 md:gap-3 whitespace-nowrap flex-shrink-0">
          ...
          <p className="text-[8px] md:text-[12px] tracking-wide">
            <span className="font-extrabold text-blue-400">{tb.promoLabel}</span>
            <span className="text-white font-bold underline decoration-blue-500/50 underline-offset-2 ml-0.5 md:ml-1">{tb.promoText}</span>
          </p>
        </div>
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          >
            <span className="mr-10 md:mr-20">{tb.marqueeText}</span>
            <span>{tb.marqueeText}</span>
          </div>
        </div>
          ...
          {user ? (
            ...
            >
              {user.role === "partner" || user.role === "admin" ? (
                <><FaThLarge className="text-[6px] md:text-[10px]" /> {tb.profileButtonText}</>
              ) : (
                <><FaUser className="text-[6px] md:text-[10px]" /> {tb.accountButtonText}</>
              )}
            </button>
          ) : (
            <button 

...
const AllProductsPage = () => {
  ...
  useEffect(() => {
    const fetchSEO = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/seo/products`);
  ...
  const getPageTitle = () => {
    if (subcategoryFilter) return `${subcategoryFilter} Products | Industrial Marketplace`;
    if (categoryFilter) return `${categoryFilter} Products | Industrial Marketplace`;
    return "All Industrial Products | Manufacturing Marketplace";
  };

  const getPageDescription = () => {
    if (subcategoryFilter) return `Browse high-quality ${subcategoryFilter} products from verified manufacturers. Get best prices and direct supplier contact.`;
    if (categoryFilter) return `Explore premium ${categoryFilter} products for industrial needs. Verified suppliers, competitive pricing, and nationwide delivery.`;
    return "Discover top industrial products, machinery, and equipment from verified manufacturers across India. Get quotes, compare prices, and connect directly with suppliers.";
  };

...
const SliderTab = ({ onRefresh }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      let url = `${API_URL}/sliders`;
      let method = "POST";
      
      if (editingId) {
        url = `${API_URL}/sliders/${editingId}`;
        method = "PUT";
      }
      ...
      if (formData.highlight) formDataObj.append("highlight", formData.highlight.trim());
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.msg && data.msg.includes("deleted")) {
  ...
  return (
    <div className="bg-[#0f1724] rounded-xl p-6 h-full flex flex-col">
      ...
      <div className="flex justify-between items-center mb-6 flex-shrink-0">
        <div>
          <h2 className="text-xl font-semibold text-white">Banner Slider Manager</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., automatic cup machine"
                    required
                  />
                </div>
                <div>
                  <label className="block text-gray-300 text-sm mb-1">Highlight (optional)</label>
                  <input
                    type="text"
                    name="highlight"
                    ...
                    onChange={handleChange}
                    className="w-full p-2 bg-[#0f1724] border border-white/10 rounded-lg text-white focus:outline-none focus:border-orange-500"
                    placeholder="e.g., High-efficiency cup production"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                {editingId && !imageFile && formData.image && (
                  <p className="text-gray-400 text-xs mt-1">Current image will be kept if no new file selected</p>
                )}
              </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {slide.highlight && (
                          <span className="text-orange-400 text-sm">{slide.highlight}</span>
                        )}
